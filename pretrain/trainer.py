...
from pretrain.datasets.dataset_pretrain import Dataset, create_ops
...
class Trainer(object):
    ...
    def get_model_class(model_type='proposed'):
        if model_type == 'proposed':
            from pretrain.model_proposed import Model
        elif model_type == 'wordnet':
            from pretrain.model_wordnet import Model
        elif model_type == 'description':
            from pretrain.model_description import Model
        elif model_type == 'separable':
            from pretrain.model_separable import Model
        return Model
    ...
    def __init__(self, config, dataset):
        ...
        self.train_dir = './train_dir/{}_{}_{}_seed{}'.format(
            config.model_type, config.prefix, hyper_parameter_str,
            config.seed)
        ...
        with tf.name_scope('datasets/batch'):
            ...
            batch_opt = {
                tf.equal(self.target_split, 'train'): lambda: pretrain_batch['train'],
                tf.equal(self.target_split, 'val'): lambda: pretrain_batch['val']
            }
            self.batch = tf.case(
                batch_opt, default=lambda: pretrain_batch['train'], exclusive=True)
...
def main():
    ...
    parser.add_argument('--data_dir', type=str,
                        ...
                        help=' ')
    parser.add_argument('--wordset_dict_name', type=str,
                        default='wordset_dict5_depth0.pkl', help=' ')
    ...
    parser.add_argument('--expand_depth', type=str2bool, default=False,
                        help='whether to use depth-based wordsets')
    # model parameters
    ...
    parser.add_argument('--model_type', type=str, default='proposed', help=' ',
                        choices=['proposed',
                                 'wordnet',
                                 'description',
                                 'separable'])
